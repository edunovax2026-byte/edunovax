...
async function initializeNotesSystem() {
    ...
    try {
        ...
        allSubjects.forEach((subject, index) => {
            ...
            // 🌟 FIX: Removed inline opacity to ensure subjects are always visible 🌟
            groupDiv.className = 'subject-group fade-in-up'; 
            ...
            if (note.type === 'link') {
                ...
                readBtn.onclick = () => {
                    ...
                    document.getElementById('text-note-body').innerHTML = `
                        <div class="note-viewer-box" style="position: relative; width: 100%; border-radius: 5px; overflow: hidden; background: #fff; margin-top: 15px; border: 1px solid #bdc3c7; height: 80vh;">
                    ...
                    if(note.type === 'image') {
                        document.getElementById('text-note-body').innerHTML = `
                            <div style="text-align: center; user-select: none; pointer-events: none; margin-top: 15px; width: 100%;">
                                <img src="${note.content}" class="note-viewer-img" style="max-width: 100%; height: auto; max-height: 80vh; border-radius: 5px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);" alt="Note Image">
                            </div>`;
                    } else {
                        document.getElementById('text-note-body').innerHTML = `
                            <div class="note-viewer-text" style="text-align: left; width: 100%; color: #333; margin-top: 15px; line-height: 1.8; max-height: 80vh; overflow-y: auto;">
